      ...
      <div class="row">
        <div class="col-md-11">
           <div class="box box-primary">
            ...
            <form action="request-display" method="POST">
                ...
                <table class="table no-margin row text-center">
                 ...
                 @foreach($result as $i)         
                  <tr>
                    <td>{{$i->r_id}}</td>
                    <td>{{$i->sp_name}}</td>
                    <td>{{$i->on_behalf_of}}</td>
                    <td>{{$i->police_station}}</td>
                    <td><b>{{$i->subject}}</b></td>
                    <td><textarea rows="2" cols="35" style="border: none; background-color:#FFF !important;" disabled>{{$i->details}}</textarea></td>
                    <td>{{ Carbon\Carbon::parse($i->submit_date)->format('d-m-Y') }}</td>
